...
def hermes_launch_args(
    terminal_launch_args: object,
    *,
    model_override: object = None,
    reasoning_effort: object = None,
) -> list[str]:
    """Build the Hermes command line, appending the pre-launch picker's picks.

    Both selections are invocation-scoped in Hermes, so they travel as launch
    flags: ``model_override`` becomes ``--model`` (the provider qualifier the
    picker's catalog ids carry is stripped — Hermes resolves the provider from
    its own config) and ``reasoning_effort`` becomes ``--reasoning``.

    User pass-through args always win: a bare ``-m``/``--model``/``--reasoning``
    (or their ``=value`` joined form) already present means the user pinned
    their own launch line, so nothing is injected and the args pass through
    untouched.
    """
    args = [str(arg) for arg in (terminal_launch_args or [])]  # type: ignore[union-attr]
    pinned_model = any(arg in {"-m", "--model"} or arg.startswith("--model=") for arg in args)
    pinned_reasoning = any(
        arg in {"--reasoning"} or arg.startswith("--reasoning=") for arg in args
    )
    if model_override and not pinned_model:
        args.extend(["--model", str(model_override).split("/", 1)[-1]])
    if reasoning_effort and not pinned_reasoning:
        args.extend(["--reasoning", str(reasoning_effort)])
    return args
